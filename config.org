...
btw,my main reason for using emacs is that I don't have the right tools on Linux to write my manuscripts. So, the reason for using witchmacs is largely because of being too lazy to toss.
* Quick install
#+BEGIN_SRC 
git clone --depth=1 https://github.com/snackon/Witchmacs ~/.emacs.d
#+END_SRC
...
#+END_SRC
** and vterm
I mean,if u have to be serious, I actually prefer to use vterm-toggle
(Still is CentaurEmacs)
#+BEGIN_SRC emacs-lisp
  (use-package vterm
	:ensure t
...
#+BEGIN_SRC emacs-lisp
  (use-package company
    ...
    :hook
    ((markdown-mode lsp-mode) . company-mode))
#+END_SRC
